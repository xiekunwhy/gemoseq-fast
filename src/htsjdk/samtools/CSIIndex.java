    ...
    public BAMFileSpan getSpanOverlapping(int referenceIndex, int startPos, int endPos) {
        ...
        if(startPos > 0) {
            /** Compute 'minimumOffset' by searching the lowest level bin containing 'startPos'.
                ...
                NOTE: for whole-reference queries (startPos<=0) the walk is skipped entirely --
                on depth>=6 CSIs it could anchor on level-1 bins holding the chromosome tail
                and silently truncate everything before it.
             */
