...
public class SplicingGraph {
		...
		for(int i=0;i<this.adjList.length;i++) {
			Collections.sort(this.adjList[i],new Comparator<ComparableElement<Integer,Integer>>() {
				...
				public int compare(ComparableElement<Integer,Integer> o1, ComparableElement<Integer,Integer> o2) {
					return Integer.compare(SplicingGraph.this.introns[o2.getWeight()].wReads,SplicingGraph.this.introns[o1.getWeight()].wReads);
				}
	...
	private void addReads(Region reg, int minIntronLength, boolean longReads) throws CloneNotSupportedException {
		...
		ArrayList<ReadGroup> list = reg.getGroups();
		this.groupW = reg.getGroupWeights();
		this.strands = reg.getGroupStrands();
		this.numGroups = reg.getNumGroups();

		for(ReadGroup g : list) {
			for(int f=0;f<g.frags.size();f++) {
				for(ReadGroup.Mate m : g.frags.get(f)) {
					if(m.alignEnd < regionStart || m.alignStart > regionStart+contRegs[contRegs.length-1].regionEnd) {
						continue;
					}
					addRead(m, g.gid, minIntronLength, intronReads, intronMap,
		...
		for(int i=0;i<contRegs.length;i++) {
			...
			contRegs[i].setSortedReads(regReads[i].toArray(), firstReads[i].toArray(), lastReads[i].toArray(), wsum(regReads[i]));
		}

		for(int i=0;i<introns.length;i++) {
			intronReads[i].sortAndMakeUnique();
			introns[i].readIdxs = intronReads[i].toArray();
			introns[i].wReads = wsum(introns[i].readIdxs);
		}


	}

	private int wsum(IntList idxs) {
		int s = 0;
	...
	private void addRead(ReadGroup.Mate m, int readIdx, int minIntronLength, IntList[] intronReads, HashMap<IntronKey,Integer> intronMap,
		...
		for(int k=0;k<m.bRef.length;k++) {
			...
			if(lastRelEnd > -1 && relStart >= 0) {
				if(relStart-lastRelEnd < minIntronLength) {
					...
					key.setStartEnd(lastRelEnd+m.delBefore[k], relStart-m.delOff[k]);
					if(intronMap.containsKey(key)) {
						int idx = intronMap.get(key);
						intronReads[idx].add(readIdx);
					}else {
			...
			for(int i=Math.max(0,-relStart);i<m.bLen[k] && relStart+i < regions.length;i++) {
	...
	public LinkedList<Transcript> quantify(LinkedList<Transcript> original, double percentExplained, double minReadsPerTranscript, double maxFraction, 
			...
			if(introns[i].getStart()+1<introns[i].getEnd()) {
				...
				for(int k=0;k<contRegs.length;k++) {
					if(isAlternative[k][i]) {
						double num = contRegs[k].getNumberOfReads();
						if(num>maxNum) {
							maxNum = num;
						}
					}
				}
				double fac = 2.0*introns[i].wReads/(maxNum+introns[i].wReads);
		...
		for(i=0;i<contRegs.length;i++) {
			...
			for(int k=0;k<introns.length;k++) {
				if(isAlternative[i][k]) {
					double num = introns[k].wReads;
					if(num > maxNum) {
						maxNum = num;
					}
				}
			}
			double fac = 2.0*contRegs[i].getNumberOfReads()/(maxNum+contRegs[i].getNumberOfReads());
		...
		for(i=0;i<readWeights.length;i++) {
				if(readNs[i] > 0) {
					readWeights[i] = Math.pow(readWeights[i], 1.0/readNs[i]) * groupW[i];
				}else {
					readWeights[i] = 0.0;
				}
		}
	...
	public void enumerateTranscripts(LinkedList<Transcript> results, double minReads, double minFraction) {
		...
		for(int i=0;i<contRegs.length;i++) {
			if(contRegs[i].left.size() == 0) {
				starts.add( new ComparableElement<Integer, Double>(i, -(double)contRegs[i].getNumberOfReads()) );
			}
		}
	...
	private void enumerate(LinkedList<Integer> exons, LinkedList<Integer> introns, LinkedList<Transcript> results, double minReads, double minFraction) {
		...
		if( adjList[exons.getLast()].size() == 0 ) {
			if(contRegs[ exons.getLast() ].getNumberOfReads()>=minReads) {
			...
			for(ComparableElement<Integer,Integer> liEl : adjList[exons.getLast()]) {
				if(this.introns[ liEl.getWeight() ].wReads >= minReads
						&& contRegs[ liEl.getElement() ].getNumberOfReads() >= minReads) {
						...
						double inter = this.introns[liEl.getWeight() ].wReads;
				...
				}else if(contRegs[ liEl.getElement() ].getNumberOfReads() >= minReads) {
					if(contRegs[ liEl.getElement() ].getNumberOfReads() > numForLater) {
						numForLater = contRegs[ liEl.getElement() ].getNumberOfReads();
	...
	public void enumerateLongReadTranscripts(LinkedList<Transcript> results, double minReads) {
		...
		// number of distinct read groups (rows of the support matrix)
		int totalNumReads = numGroups;
		...
		public String toString() {
			...
			if(SplicingGraph.this.introns.length==0 || SplicingGraph.this.introns[0].readIdxs == null) {
				...
				for(i=0;i<introns.size();i++) {
					int temp = SplicingGraph.this.introns[introns.get(i)].wReads;
					tie +=  temp > 0 ? 1 : 0;
			...
			for(i=0;i<introns.size();i++) {
				int temp = 0;
				if(SplicingGraph.this.introns.length!=0 && SplicingGraph.this.introns[0].readIdxs != null) {
					temp = SplicingGraph.this.introns[introns.get(i)].wReads;
				}
				if(temp < minCov) {
					minCov = temp;
				}
	...
	private class ContiguousRegion{
		...
		public void setSortedReads(int[] readIdxs, int[] firstReadIdxs, int[] lastReadIdxs, int wReads) {
			this.readIdxs = readIdxs;
			this.numReads = wReads;
